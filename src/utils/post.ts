...
export function filterByLanguage(posts: Array<CollectionEntry<"blog" | "projects">>, lang: SupportedLanguage): Array<CollectionEntry<"blog" | "projects">> {
	return posts.filter((post) => {
		const translationLang = post.data.slug ? getLangFromSlug(post.data.slug) : undefined;
...
export function getSlugFromCollectionEntry(entry: CollectionEntry<"blog" | "projects">) {
	const [, ...slugs] = (entry.data.slug ?? "").split("/");
	// if collection entry is a translation, grab the slug only
	return slugs.length ? slugs.join("/") : entry.data.slug;
}
